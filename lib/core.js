                        ...
                        if(iszuiyou){
                            ...
                            if(list){
                                for(var i in list){
                                    ...
                                    var topic = {content:item.content,votesCount:item.likes};
                                    if(item.imgs&&item.imgs.length>0){
                                        // 保存图片？
                                        // console.log(item.imgs);
                                        topic.imgs = JSON.stringify(item.imgs);
                                    }
                                    // 特么的videos不是数组，是对象。。。
                                    if(item.videos){
                                        // 保存视频？
                                        topic.videos = JSON.stringify(item.videos);
                                    }
                                    model.Topics.create(topic).then(function(result,error){
                                        // console.log(result);
                                        if(error)
                                        console.log(error);
                                    });
